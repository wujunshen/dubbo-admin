...
public class RegistryServerSyncTest {
  ...
  @InjectMocks private RegistryServerSync registryServerSync;
  ...
  public void testNotify() {
    ...
    when(consumerUrl.getParameter(Constants.CATEGORY_KEY, Constants.PROVIDERS_CATEGORY))
        .thenReturn(Constants.CONSUMER_PROTOCOL);
    ...
    when(consumerUrl.toFullString())
        .thenReturn(
            "consumer://192.168.1.10/sunbufu.dubbo.consumer?application=dubbo&category=consumer&check=false&dubbo=2.7.0&interface=sunbufu.dubbo.consumer&loadbalabce=roundrobin&mehods=sayHi,sayGoodBye&owner=sunbufu&pid=18&protocol=dubbo&side=consumer&timeout=3000&timestamp=1548127407769");
    when(providerUrl.getParameter(Constants.CATEGORY_KEY, Constants.PROVIDERS_CATEGORY))
        .thenReturn(Constants.PROVIDER_PROTOCOL);
    ...
    when(providerUrl.toFullString())
        .thenReturn(
            "consumer://192.168.1.10/sunbufu.dubbo.consumer?application=dubbo&category=consumer&check=false&dubbo=2.6.2&interface=sunbufu.dubbo.consumer&loadbalabce=roundrobin&mehods=sayHi,sayGoodBye&owner=sunbufu&pid=18&protocol=dubbo&side=consumer&timeout=3000&timestamp=1548127407769");

    registryServerSync.notify(Arrays.asList(consumerUrl, consumerUrl, providerUrl));

    ConcurrentMap<String, Map<String, URL>> consumerMap =
        registryServerSync.getRegistryCache().get(Constants.CONSUMER_PROTOCOL);
    assertTrue(consumerMap.containsKey("org.apache.dubbo.consumer"));
    ConcurrentMap<String, Map<String, URL>> providerMap =
        registryServerSync.getRegistryCache().get(Constants.PROVIDER_PROTOCOL);
