...
public class ConditionRoutesControllerTest extends AbstractSpringIntegrationTest {
  ...
  public void shouldThrowWhenParamInvalid() {
    ...
    assertThat(
        ...
        containsString("dubbo 2.6 does not support application scope routing rule"));
  }
  ...
  public void shouldUpdateRule() throws Exception {
    ...
    List<String> newConditions =
        Arrays.asList("=> host != 172.22.3.211", "=> host != 172.22.3.212");
    ...
    ResponseEntity<String> responseEntity =
        restTemplate.exchange(
            url("/dubbo-admin/api/{env}/rules/route/condition/{service}"),
            HttpMethod.PUT,
            new HttpEntity<>(dto, null),
            String.class,
            env,
            service);
  ...
  public void shouldGetServiceRule() throws Exception {
    ...
    ResponseEntity<List<ConditionRouteDTO>> responseEntity =
        restTemplate.exchange(
            url("/dubbo-admin/api/{env}/rules/route/condition/?service={service}"),
            HttpMethod.GET,
            null,
            new ParameterizedTypeReference<List<ConditionRouteDTO>>() {},
            env,
            service);
    assertThat(responseEntity.getStatusCode(), is(HttpStatus.OK));
    ...
    assertThat(
        conditions, containsInAnyOrder("=> host != 172.22.3.111", "=> host != 172.22.3.112"));
  }
  ...
  public void shouldDeleteRule() throws Exception {
    ...
    assertNotNull(
        "zk path should not be null before deleting", zkClient.checkExists().forPath(path));
  ...
  public void shouldThrowWhenDetailRouteWithUnknownId() {
    ResponseEntity<String> responseEntity =
        restTemplate.getForEntity(
            url("/dubbo-admin/api/{env}/rules/route/condition/{id}"),
            String.class,
            env,
            "non-existed-service");
  ...
  public void shouldGetRouteDetail() throws Exception {
    ...
    assertThat(
        conditionRouteDTO.getConditions(),
        containsInAnyOrder("=> host != 172.22.3.111", "=> host != 172.22.3.112"));
  }
  ...
  public void shouldEnableRoute() throws Exception {
    ...
    restTemplate.put(
        url("/dubbo-admin/api/{env}/rules/route/condition/enable/{id}"), null, env, service);
  ...
  public void shouldDisableRoute() throws Exception {
    ...
    restTemplate.put(
        url("/dubbo-admin/api/{env}/rules/route/condition/disable/{id}"), null, env, service);
