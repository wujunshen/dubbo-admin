...
public class ManagementControllerTest extends AbstractSpringIntegrationTest {
  ...
  public void shouldThrowWhenUpdateNonExistedConfigKey() {
    ...
    ResponseEntity<Void> responseEntity =
        restTemplate.exchange(
            url("/dubbo-admin/api/{env}/manage/config/{key}"),
            HttpMethod.PUT,
            new HttpEntity<>(configDTO),
            Void.class,
            env,
            "non-existed");
  ...
  public void shouldGetAllConfig() throws Exception {
    ...
    ResponseEntity<List<ConfigDTO>> responseEntity =
        restTemplate.exchange(
            url("/dubbo-admin/api/{env}/manage/config/{key}"),
            HttpMethod.GET,
            null,
  ...
  public void shouldDeleteConfig() throws Exception {
    ...
    restTemplate.delete(
        url("/dubbo-admin/api/{env}/manage/config/{key}"), env, "shouldDeleteConfigKey1");
    ResponseEntity<List<ConfigDTO>> responseEntity =
        restTemplate.exchange(
            url("/dubbo-admin/api/{env}/manage/config/{key}"),
            HttpMethod.GET,
    ...
    restTemplate.delete(
        url("/dubbo-admin/api/{env}/manage/config/{key}"), env, "shouldDeleteConfigKey10");
    responseEntity =
        restTemplate.exchange(
            url("/dubbo-admin/api/{env}/manage/config/{key}"),
            HttpMethod.GET,
  ...
  private String getPath(String key) {
    key =
        StringUtils.isNotEmpty(key)
            ? key + Constants.PATH_SEPARATOR + Constants.DUBBO_PROPERTY
