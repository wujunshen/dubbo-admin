  ...
  private String doGetMetaData(MetadataIdentifier key) {
    try {
      Response<GetValue> response =
          this.client.getKVValue(key.getUniqueKey(MetadataIdentifier.KeyTypeEnum.UNIQUE_KEY));
      return response.getValue().getDecodedValue();
    } catch (Exception e) {
      LOG.error(
          String.format(
              "Failed to fetch metadata for %s from consul, cause: %s",
              key.getUniqueKey(MetadataIdentifier.KeyTypeEnum.UNIQUE_KEY), e.getMessage()),
          e);
    }
    return null;
  }
